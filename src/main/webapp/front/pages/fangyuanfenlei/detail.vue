...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','fangyuanfenlei');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','fangyuanfenlei');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('fangyuanfenlei', this.id);
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discussfangyuanfenlei/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('fangyuanfenlei', this.detail);
				this.$utils.msg('审核成功');
